...
import { TabsService, TabOptions } from '@app/tabs/tabs.service';
...
export const initTabs = (tabService: TabsService, tabs: Array<TabOptions>) => {
    tabService.removeAllTabs();

    /* istanbul ignore else  */
    if (tabs && tabs.length > 0) {
        tabs.forEach(tabOptions => {
            tabService.addTab(tabOptions);
        });
    }
};
